...
export async function getAllQuotes(): Promise<Quote[]> {
  const result = await pool.query(
    `SELECT * FROM qq_quotes WHERE deleted_at IS NULL ORDER BY submission_date DESC, created_at DESC`
  );
...
export async function getQuoteById(id: string): Promise<Quote | undefined> {
  ...
  const result = await pool.query(
    `SELECT * FROM qq_quotes WHERE deleted_at IS NULL AND (id::text = $1 OR quote_number = $1)`,
    [id]
  );
...
export async function getQuotesByEmail(email: string): Promise<Quote[]> {
  const result = await pool.query(
    `SELECT * FROM qq_quotes WHERE deleted_at IS NULL AND LOWER(customer_email) = LOWER($1)
...
export async function getQuotesByCustomerId(customerId: string): Promise<Quote[]> {
  const result = await pool.query(
    `SELECT * FROM qq_quotes WHERE deleted_at IS NULL AND customer_id = $1
     ORDER BY submission_date DESC, created_at DESC`,
    [customerId]
  );
  return result.rows.map(rowToQuote);
